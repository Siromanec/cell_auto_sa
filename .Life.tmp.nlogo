...
to setup-csv
  ...
  set last-ticks-data (n-values window-size [0])
  let figure csv:from-row file-read-line
  print figure
  ;(foreach (range 0 (length figure - 1) 2) [[i] -> ask patch (item i figure) (item (i + 1) figure) [cell-birth print (item i figure) print (item (i + 1) figure) print "------------"]])
  (foreach (range 0 (length figure - 1) 2) [[i] -> ask patch (item i figure) (item (i + 1) figure) [cell-birth]])
  log-data
end
...
to log-data
  set last-ticks-data replace-item (ticks mod window-size) last-ticks-data (count patches with [living?])
end

to-report report-steady-state?
  let diff ((max last-ticks-data) - (min last-ticks-data))
  if diff <= steady-state-threshold [
    set steady-state-count count patches with [living?]
    report true
  ]
  report false
end
...
to go-csv
  if not any? patches with [living?] [stop]
  if (report-steady-state? and ticks > terminate-after-at-least-nticks) [stop]
  go
  log-data
end
...
279
506
terminate-after-at-least-nticks
terminate-after-at-least-nticks
0
200
50.0
...
@#$#@#$#@
<experiments>
  <experiment name="run-for-6" repetitions="500" runMetricsEveryStep="true">
